...
export class DatabaseMigrator {
    ...
    try {
      ...
      await this.db.begin(async (sql) => {
        for (const migration of pendingMigrations) {
          console.log(`⏳ Running migration: ${migration.id}_${migration.name.replace(/ /g, "_")}`);
          ...
          console.log(`✅ Completed migration: ${migration.id}_${migration.name.replace(/ /g, "_")}`);
        }
      });

      console.log(`🎉 Successfully ran ${pendingMigrations.length} migration(s) in batch ${batch}`);
    } catch (error) {
  ...
  async rollback(): Promise<void> {
    ...
    try {
      ...
      console.log(`🔄 Rolling back batch ${latestBatch} (${batchMigrations.length} migration(s))...`);
      ...
      await this.db.begin(async (sql) => {
        for (const record of batchMigrations) {
          ...
          console.log(`⏳ Rolling back: ${record.id}_${record.name.replace(/ /g, "_")}`);
        ...
        `;
          ...
          console.log(`✅ Rolled back: ${record.id}_${record.name.replace(/ /g, "_")}`);
        }
      });
  ...
  async status(): Promise<void> {
    ...
    for (const migration of allMigrations) {
      ...
      console.log(`${status} ${migration.id}_${migration.name.replace(/ /g, "_")}${executedInfo}`);
    }
