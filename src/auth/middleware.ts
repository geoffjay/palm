...
import { type SessionData, SessionManager } from "./session";
...
export type RouteHandler = (req: AuthenticatedRequest) => Promise<Response> | Response;
...
export class AuthMiddleware {
   ...
   */
  async authenticate(request: AuthenticatedRequest): Promise<AuthenticatedRequest> {
  ...
  requireAuth(handler: AuthenticatedRouteHandler): RouteHandler {
    return async (req: AuthenticatedRequest) => {
      ...
      return handler(authenticatedReq as AuthenticatedRequest & { user: SessionData });
    };
  }
   ...
   */
  rateLimit(maxRequests: number, windowMs: number): (handler: RouteHandler) => RouteHandler {
